...
sml <- read.csv("Table/Sampleinfo.csv", header=T, row.names=1)
dim(sml)
sml['S_1721', 'DiseasePlant'] <- NA # S_1721 Plant Disease Sore =110: possible mistyping of 11.1

#rownames(sml) <- gsub("S_0", "S_", rownames(sml))
#head(sml)
...
sml$longitude_east[which(sml$longitude_east=="138.00.23.6")] <- "138.00.24"
...
###### Apply below, if sites with less than 10 samples shoudl be removed. #####
...
smlnew <- cbind(smltmp[,c(expV, 'DSI', 'DiseasePlant')])
...
diseaseLev <- diseaseSum <- data.frame(row.names=names(which(diseaseRow<2)), disease=rep(0,nrow(diseaseRmNA)) )
diseaseSum[,1] <- apply(diseaseRmNA, 1, max, na.rm=TRUE)

#diseaseLev[which(diseaseSum[,1]>100), ] <- NA # S_1721 Plant Disease Sore =110: possible mistyping of 11.1
...
sml0 <- cbind(smlnew, diseaseMax=diseaseSum[rownames(smlnew),], DL=diseaseLev[rownames(smlnew),1])
smlout <- sml0[order(rownames(sml0), decreasing=FALSE), ]

############################################################################

write.table(cbind(Sample.ID=rownames(smlout), smlout), file=sprintf('%s/SampleInfo_RmOutliers.txt', dir$tabledir), quote=F, sep='\t', row.names=F)

saveRDS(smlout, 'Table/0301_sml.rds')
