...
g[[4]] <- ggplot(SS3, aes(x=as.numeric(PCoA1), y=as.numeric(PCoA2), fill= SSentropy))+
        geom_point(shape=21, alpha=0.8)+
        scale_fill_gradientn(colors=brewer.pal(8, 'RdPu'), na.value = NA)+
        theme_bw(base_size=15)+
...
colpl <- c(rev(brewer.pal(7, "Blues")), "oldlace  ", "mistyrose", "red")
...
mod4 <- gam(SSentropy ~ s(PCoA1, PCoA2), data= SS3)
vis.gam(mod4, ticktype='detailed', color="cm", theta=20, phi=60, main="Stable-state entropy")
