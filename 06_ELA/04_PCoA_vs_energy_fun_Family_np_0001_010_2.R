...
dir.create('06_ELA/04_output_fun_Family')
dir <- make.dir(sprintf('06_ELA/04_output_fun_Family/rel%s_occ%s', rel, occ))
...
mat <- readRDS("Table/ELA_matrix_each_fun_Family.rds")
mat <- mat/rowSums(mat)

env <- readRDS("Table/env_fun_1664.rds") 
env <- data.frame(SampleID=rownames(env), env)

eng <- read.csv(sprintf("06_ELA/02_output_fun_Family/rel%s_occ%s/Table/gStability_meanEnv_fun_Family_1664_%s_%s.csv", rel, occ, rel, occ), header=T, row.names=1)
eng <- data.frame(SampleID=rownames(eng), eng)

#table(eng$stable.state.id.np)

pcoa.env <- left_join(pca_fng, env, by="SampleID")
df <- left_join(pcoa.env, eng, by="SampleID")

write.csv(df, sprintf('%s/Engergy_fun_Family_1664_%s_%s.csv', dir$tabledir, rel, occ))
...
sml <- readRDS("Table/sml_fun_1664.rds") 
...
#colpl <- c("blue", rev(brewer.pal(3, "Blues")), 'mistyrose', "lightpink", "deeppink2", "red")
#colpl <- c("skyblue1", "red")
colpl <- c("blue", "skyblue1", "mistyrose", "lightpink", "deeppink1")
...
ss <- readRDS(sprintf("Table/gStability_meanEnv_fun_Family_1664_%s_%s.rds", rel, occ))[[3]][[2]]
ss2 <- ss[,3:ncol(ss)]
rownames(ss2) <- ss$ID
...
levelplot(ss9[stateDLreorder$ID, sort(colnames(ss9), decreasing=T)], xlab="Stable state ID", ylab="Family",
          scales = list(x = list(rot = 45), cex = 0.8), col.regions=colorRampPalette
          (c("white", "deeppink3 ")))      
dev.off()
...
vis.gam(mod2, ticktype='detailed', color="bw", theta=20, phi=60, n.grid=40, zlab="energy")

mod1 <- gam(DL.bin ~ s(PCoA.1, PCoA.2), data= df2)
vis.gam(mod1, ticktype='detailed', color="heat", theta=20, phi=60, n.grid=40, zlab="prop. disease severity level < 20")
