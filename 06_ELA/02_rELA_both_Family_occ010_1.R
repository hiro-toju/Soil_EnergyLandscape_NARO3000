...
# loading data
baseabtable <- readRDS('Table/ELA_matrix_each_both_Family.rds')
env <- readRDS('Table/env_both_1664.rds')
...
pdf(file=sprintf("%s/showDG1_gela_both_Family_1664_%s_%s_prun%s.pdf", dir$figdir, rel, occ, prun))
showDG(gela[[1]][[1]][[1]], ocmat)
dev.off()

pdf(file=sprintf("%s/showDG2_gela_both_Family_1664_%s_%s.pdf", dir$figdir, rel, occ))
showDG(gela[[1]][[2]][[1]], ocmat)
dev.off()

pdf(file=sprintf("%s/showSSD_gela_both_Family_1664_%s_%s_%s.pdf", dir$figdir, rel, occ, prun))
showSSD(gela)
dev.off()
