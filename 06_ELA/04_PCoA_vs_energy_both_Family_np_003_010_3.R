...
dir.create('06_ELA/04_output_both_Family')
dir <- make.dir(sprintf('06_ELA/04_output_both_Family/rel%s_occ%s', rel, occ))

############################################################################
# -- Load data table

#mat <- readRDS("Table/ELA_matrix_each_both_Family.rds")
m.pro <- readRDS('Table/ELA_matrix_pro_Family.rds')
m.fun <- readRDS('Table/ELA_matrix_fun_Family.rds')
...
saveRDS(pca, 'Table/PCoA_Family_both_rrarefy.rds')

env <- readRDS('Table/env_1474.rds')
env <- data.frame(SampleID=rownames(env), env)

eng <- read.csv(sprintf("06_ELA/02_output_both_Family/rel%s_occ%s/Table/gStability_meanEnv_both_Family_1664_%s_%s.csv", rel, occ, rel, occ), header=T, row.names=1)
...
write.csv(df, sprintf('%s/Engergy_both_Family_%s_%s.csv', dir$tabledir, rel, occ))
...
ggsave(plot=plot_grid( plotlist=g, ncol=2), w= 9, h= 9,
           filename=sprintf('%s/EnergyIndices_PCoA_Family_%s_%s.pdf', dir$figdir, rel, occ))  
...
ggsave(plot=plot_grid( plotlist=g, ncol=2), w= 11.5, h= 9,
       filename=sprintf('%s/EnergyIndices_PCoA_Family_%s_%s_caption.pdf', dir$figdir, rel, occ))  
...
write.table(stateDLreorder, sprintf('%s/StableStates_MealDL_Family_%s_%s.csv', dir$tabledir, rel, occ))
...
#colpl <- c("blue4", rev(brewer.pal(8, "Blues")), 'mistyrose', "lightpink", "deeppink2", "red", "brown4")
#colpl <- c("blue4", rev(brewer.pal(7, "Blues")), brewer.pal(8, "Reds"),  "brown4")
#colpl <- c("blue", "skyblue1", "lightpink", "deeppink1")
#colpl <- c("blue", "skyblue1",  "deeppink1")
colpl <- c("blue", "skyblue1", "lightpink", "deeppink1", "red")
...
ggsave(g, w= 7, h= 3, filename=sprintf('%s/Barplot_StableStates_DL_Family_%s_%s.pdf', dir$figdir, rel, occ))  
############################################################################
# Stable states 
...
comm6 <- comm6[c(sort(rownames(comm6[290:470,]), decreasing=T), sort(rownames(comm6[1:289,]), decreasing=T)), ]


############################################################################
# levelplot all taxa

pdf(file=sprintf('%s/Levelplot_StableStates_Family_continuous_%s_%s.pdf', dir$figdir, rel, occ), width=6, height=8)
levelplot(as.matrix(t(comm6[, stateDLreorder$ID])), xlab="Stable state ID", ylab="Family",
          scales = list(x = list(rot = 45), cex = 0.8), col.regions=colorRampPalette
(c("white", "deeppink3 ")))      
dev.off()

pdf(file=sprintf('%s/Levelplot_StableStates_Family_continuous_incUnidentified_%s_%s.pdf', dir$figdir, rel, occ), width=6, height=8)
levelplot(as.matrix(t(comm5[, stateDLreorder$ID])), xlab="Stable state ID", ylab="Family",
          scales = list(x = list(rot = 45), cex = 0.8), col.regions=colorRampPalette
          (c("white", "deeppink3 ")))      
dev.off()
...
ss <- readRDS(sprintf("Table/gStability_meanEnv_both_Family_1664_%s_%s.rds", rel, occ))[[3]][[2]]
ss2 <- ss[,3:ncol(ss)]
rownames(ss2) <- ss$ID
...
pdf(file=sprintf('%s/Levelplot_StableStates_Family_binary_%s_%s.pdf', dir$figdir, rel, occ), width=6, height=8)
levelplot(t(ss5[order(rownames(ss5), decreasing=T),]), xlab="Stable state ID", ylab="Family",
          scales = list(x = list(rot = 45), cex = 0.8), col.regions=colorRampPalette
          (c("white", "deeppink3 ")))      
dev.off()
...
ss9 <- ss9[, c(sort(colnames(ss9[ ,9:12]), decreasing=T), sort(colnames(ss9[ ,1:8]), decreasing=T))]

pdf(file=sprintf('%s/Levelplot_StableStates_Family_binary_majorSS_%s_%s.pdf', dir$figdir, rel, occ), width=6, height=8)
levelplot(ss9[stateDLreorder$ID, -1], xlab="Stable state ID", ylab="Family",
          scales = list(x = list(rot = 45), cex = 0.8), col.regions=colorRampPalette
...
vis.gam(mod2, ticktype='detailed', color="bw", theta=20, phi=60, n.grid=50, zlab="energy")
...
vis.gam(mod1, ticktype='detailed', color="heat", theta=20, phi=60, n.grid=50, zlab="prop. disease severity level < 20")

dev.off()

#######################################################

pdf(file=sprintf('%s/Landscapes_3D_Family_%s_%s_2.pdf', dir$figdir, rel, occ), width=12, height=12)
...
p <- persp(PCoA_1, PCoA_2, Energy, theta = 10, phi=50, col = "khaki1", ticktype="detailed", zlim=c(min(df2$e.realize), max(df2$e.realize)), zlab="energy")
...
p <- persp(PCoA_1, PCoA_2, Energy, theta = 10, phi=40, col = "khaki1", ticktype="detailed", zlim=c(min(df2$e.realize), max(df2$e.realize)), zlab="energy")
obs <- with(df2, trans3d(PCoA.1, PCoA.2, e.realize, p))
pred <- with(df2, trans3d(PCoA.1, PCoA.2, fitted(mod2), p))
points(obs, col = colors, pch = 16, cex=0.8)

p <- persp(PCoA_1, PCoA_2, Energy, theta = 10, phi=30, col = "khaki1", ticktype="detailed", zlim=c(min(df2$e.realize), max(df2$e.realize)), zlab="energy")
obs <- with(df2, trans3d(PCoA.1, PCoA.2, e.realize, p))
pred <- with(df2, trans3d(PCoA.1, PCoA.2, fitted(mod2), p))
points(obs, col = colors, pch = 16, cex=0.8)

p <- persp(PCoA_1, PCoA_2, Energy, theta = 10, phi=20, col = "khaki1", ticktype="detailed", zlim=c(min(df2$e.realize), max(df2$e.realize)), zlab="energy")
obs <- with(df2, trans3d(PCoA.1, PCoA.2, e.realize, p))
pred <- with(df2, trans3d(PCoA.1, PCoA.2, fitted(mod2), p))
points(obs, col = colors, pch = 16, cex=0.8)
