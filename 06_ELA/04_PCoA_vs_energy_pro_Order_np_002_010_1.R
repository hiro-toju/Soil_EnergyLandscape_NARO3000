...
dir.create('06_ELA/04_output_pro_Order')
dir <- make.dir(sprintf('06_ELA/04_output_pro_Order/rel%s_occ%s', rel, occ))

############################################################################
# -- Load data table

dimrm <- readRDS('Table/PCoA_Order_rrarefy.rds')
...
mat <- readRDS("Table/ELA_matrix_each_pro_Order.rds")
mat <- mat/rowSums(mat)

env <- readRDS("Table/env_pro_1771.rds") 
env <- data.frame(SampleID=rownames(env), env)

eng <- read.csv(sprintf("06_ELA/02_output_pro_Order/rel%s_occ%s/Table/gStability_meanEnv_pro_Order_1771_%s_%s.csv", rel, occ, rel, occ), header=T, row.names=1)
...
write.csv(df, sprintf('%s/Engergy_pro_Order_1771_%s_%s.csv', dir$tabledir, rel, occ))
...
#colpl <- c("blue4", rev(brewer.pal(9, "Blues")), 'mistyrose', "lightpink", "deeppink2", "red",  "brown4")
colpl <- c("blue4", rev(brewer.pal(4, "Blues")), brewer.pal(4, "Reds"),  "brown4")
...
ss <- readRDS(sprintf("Table/gStability_meanEnv_pro_Order_1771_%s_%s.rds", rel, occ))[[3]][[2]]
ss2 <- ss[,3:ncol(ss)]
rownames(ss2) <- ss$ID
...
levelplot(ss9[stateDLreorder$ID, sort(colnames(ss9), decreasing=T)], xlab="Stable state ID", ylab="Order",
          scales = list(x = list(rot = 45), cex = 0.8), col.regions=colorRampPalette
          (c("white", "deeppink3 ")))      
dev.off()
