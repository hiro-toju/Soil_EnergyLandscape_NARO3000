...
dir.create('06_ELA/04_output_pro_Family')
dir <- make.dir(sprintf('06_ELA/04_output_pro_Family/rel%s_occ%s', rel, occ))
...
mat <- readRDS("Table/ELA_matrix_each_pro_Family.rds")
mat <- mat/rowSums(mat)

env <- readRDS("Table/env_pro_1771.rds") 
env <- data.frame(SampleID=rownames(env), env)

eng <- read.csv(sprintf("06_ELA/02_output_pro_Family/rel%s_occ%s/Table/gStability_meanEnv_pro_Family_1771_%s_%s.csv", rel, occ, rel, occ), header=T, row.names=1)
eng <- data.frame(SampleID=rownames(eng), eng)

#table(eng$stable.state.id.np)

pcoa.env <- left_join(pca_prk, env, by="SampleID")
df <- left_join(pcoa.env, eng, by="SampleID")

write.csv(df, sprintf('%s/Engergy_pro_Family_1771_%s_%s.csv', dir$tabledir, rel, occ))
...
sml <- readRDS("Table/sml_pro_1771.rds") 
...
colpl <- c("blue", rev(brewer.pal(3, "Blues")), 'mistyrose', "lightpink", "deeppink2", "red",  "brown4")
...
ss <- readRDS(sprintf("Table/gStability_meanEnv_pro_Family_1771_%s_%s.rds", rel, occ))[[3]][[2]]
ss2 <- ss[,3:ncol(ss)]
rownames(ss2) <- ss$ID
